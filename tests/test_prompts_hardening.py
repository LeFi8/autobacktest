...
def test_system_prompt_contains_pandas_version() -> None:
    ...
    assert pd.__version__ in SYSTEM_PROMPT, (
        f"pandas version {pd.__version__} not in SYSTEM_PROMPT"
    )
...
def test_system_prompt_contains_params_rule() -> None:
    ...
    assert (
        "MUST go under" in SYSTEM_PROMPT
        or "must go under" in SYSTEM_PROMPT
        or "under the" in SYSTEM_PROMPT
    )
...
def test_system_prompt_contains_allowed_top_level_keys() -> None:
    ...
    from autobacktest.llm.prompts import SYSTEM_PROMPT, _ALLOWED_TOP_LEVEL_KEYS
